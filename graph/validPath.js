    ...
    class Sol{
        ...
        transform(edges){
            for(let i=0;i<edges.length;i++){
                ...
                if(this.hash[src]){
                    this.hash[src].push([src,dest])
                    // this.hash[edges[i][1]].push(dest,src)
                }
                else{
                    this.hash[src] = [[src,dest]]
                }
                if(this.hash[dest]){
                    this.hash[dest].push([dest,src])
                }else{
                    this.hash[dest] = [[dest,src]]
                }
            }
        }
        checkEdge(hash,currentNode,dest,visited,status){
            if(currentNode == dest){
                return true
            }
            for(let i=0;(hash[currentNode] && ( i< hash[currentNode].length));i++){
                const destNode = hash[currentNode][i][1]
                if(!visited[destNode]){
                    visited[destNode] = true
                    status = this.checkEdge(hash,destNode,dest,visited,status)
                    visited[destNode] = false
                }
            }
            return status
        }
        check(edges, src, dest){
            if(Object.values(this.hash).length == 0){
                this.transform(edges)
            }
            let status = false
            let vis = []
            vis[src] = true
            status = this.checkEdge(this.hash,src,dest,vis,status)
            return status
        }
